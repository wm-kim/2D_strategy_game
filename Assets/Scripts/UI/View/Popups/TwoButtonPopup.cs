...
namespace Minimax.UI.View.Popups
{
    public class TwoButtonPopup : PopupView
    {
        ...
        {
            SetUp(command.Message, command.LeftButtonText, command.RightButtonText, command.LeftButtonAction,
                command.RightButtonAction);
        }

        private void SetUp(string message, string leftButtonText, string rightButtonText, Action leftButtonCallback,
            Action rightButtonCallback)
        {
            ...
            m_leftButton.onClick.AddListener(() => { leftButtonCallback?.Invoke(); });
            m_rightButton.onClick.AddListener(() => { rightButtonCallback?.Invoke(); });
        }
    }
}