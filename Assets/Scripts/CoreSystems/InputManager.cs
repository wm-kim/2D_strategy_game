...
namespace Minimax.CoreSystems
{
    public class InputManager : MonoBehaviour
    {
        ...
        // 터치가 있는지 확인
        private bool IsTouching() => EnhancedTouch.Touch.activeFingers.Count > 0;
        
        // 터치 이미지 위치 설정
        private void SetTouchImagePosition(Vector2 position) => m_touchImage.transform.position = position;
        ...
        private void OnTouchEnded(EnhancedTouch.Touch activeTouch)
        {
            ...
            {
                OnTap?.Invoke(activeTouch.screenPosition);
            }
        }
        ...
        {
            if (!m_longTouchTriggered && (activeTouch.phase == TouchPhase.Stationary || activeTouch.phase == TouchPhase.Moved))
            {
