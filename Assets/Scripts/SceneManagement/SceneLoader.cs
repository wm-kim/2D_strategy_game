...
namespace Minimax.SceneManagement
{
    ...
    public class SceneLoader : NetworkBehaviour
    {
...
#if UNITY_EDITOR
        public void RequestColdStartup(SceneType sceneToLoad)
        {
            ColdStartup(sceneToLoad.ToString());
        }
#endif
        ...
        {
            var isPersistentScene = scene.name == SceneType.PersistentScene.ToString();
            var isCurrentScene    = scene.name == m_currentlyLoadedScene;
            return !isPersistentScene || isCurrentScene;
        }

#if UNITY_EDITOR
        private void ColdStartup(string sceneToLoad)
        {
        ...
        public async void LoadScene(SceneType sceneTypeToLoad, bool useNetwork = false)
        {
            ...
            else // 네트워크를 통하지 않고 씬을 로드합니다.
            {
                ...
                var asyncOperation = SceneManager.LoadSceneAsync(m_sceneToLoad, LoadSceneMode.Additive);
