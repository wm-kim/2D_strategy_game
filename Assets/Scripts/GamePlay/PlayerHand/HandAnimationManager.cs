...
namespace Minimax.GamePlay.PlayerHand
{
    public class HandAnimationManager : MonoBehaviour
    {
        ...
        private void AllocateSlotTransforms()
        {
            ...
            for (var i = 0; i < Define.MaxHandCardCount; i++)
            {
        ...
        {
            var cardAngle = cardCount <= 1 ? 0 : m_curvAngle / (cardCount - 1); // Adjust for a single card.
            cardAngle = Mathf.Min(cardAngle, m_maxBetweenAngle);

            var cardAngleOffset = (cardCount - 1) * cardAngle / 2;

            for (var i = 0; i < cardCount; i++)
            {
                var angle      = cardAngle * i - cardAngleOffset;
                var radian     = angle * Mathf.Deg2Rad;
                var baseRadian = m_baseRotation * Mathf.Deg2Rad;
                var x          = m_curvCenter.x + m_curvRadius * Mathf.Sin(baseRadian + radian);
                var y          = m_curvCenter.y + m_curvRadius * Mathf.Cos(baseRadian + radian);

                m_slotPositionList[i] = new Vector3(x, y, 0);

                // Rotate the card such that its end points to the curve's center point
                var directionToCenter = m_curvCenter - new Vector2(x, y);
                // Subtracting 90 degrees to align with the vertical
                var rotationAngle = Mathf.Atan2(directionToCenter.y, directionToCenter.x) * Mathf.Rad2Deg + 90f;
        ...
        public void TweenOpponentCardReveal(int cardUID, HandCardView handCardView)
        {
            ...
            seq.OnComplete(() => { Destroy(handCardView.gameObject); });
        }
    }
}