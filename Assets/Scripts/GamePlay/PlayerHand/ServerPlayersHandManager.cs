...
namespace Minimax.GamePlay.PlayerHand
{
    public class ServerPlayersHandManager : NetworkBehaviour
    {
        ...
        /// </summary>
        private Dictionary<int, List<int>> m_plyersCardInHand = new Dictionary<int, List<int>>();
        ...
        public void PrintAllPlayerHands()
        {
            foreach (var player in m_plyersCardInHand)
            {
                ...
                {
                    DebugWrapper.Log($"Card UID: {cardUID}, Card ID {ServerCard.CardsCreatedThisGame[cardUID].Data.CardId}");
                }
            }
        }
#endif
