...
namespace Minimax.GamePlay.GridSystem
{
    public class AStarPathFinding<T> : IPathFinding<T> where T : ICell
    {
        public List<T> FindPath(T start, T target, IGrid<T> grid)
        {
            ...
            List<T> toSearch = new List<T>();
            HashSet<T> processed = new HashSet<T>();
            
            // 모든 셀에 대한 g, h, f 값을 저장
            Dictionary<T, int> gCosts = new Dictionary<T, int>();
            Dictionary<T, int> hCosts = new Dictionary<T, int>();
            Dictionary<T, int> fCosts = new Dictionary<T, int>();
            ...
            while (toSearch.Count > 0)
            {
                ...
                // 선택된 셀의 인접한 셀들을 탐색
                foreach (T neighbor in grid.GetNeighbors(current).Where(neighbor => neighbor.IsWalkable &&
                             !processed.Contains(neighbor)))
                {
                    bool inToSearch = toSearch.Contains(neighbor);
                    int tentativeGCost = gCosts[current] + neighbor.GetDistance(current);
